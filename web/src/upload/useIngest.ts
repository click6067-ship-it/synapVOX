...
// On resolve we also call `onIngested(title)` — Task 14 wires that to refetch and
// grow the graph. Failures set 'error' and surface the backend's 413/429 message
// verbatim; everything is caught so submit NEVER throws uncaught.
...
export type UseIngest = {
  stage: IngestStage
  error: string | null
  submit: (title: string, text: string) => void
  reset: () => void
}
...
export function useIngest(project: string, onIngested: (title: string) => void): UseIngest {
  ...
  const submit = useCallback(
    (title: string, text: string) => {
      ...
      const run = async () => {
        try {
          await ingestText(project, title, text)
          if (!aliveRef.current) return
          clearTimers()
          setStage('growing')
          // Task 14 refetch/grow — must never break the ingest UX if it throws.
          try {
            onIngestedRef.current(title)
          } catch {
